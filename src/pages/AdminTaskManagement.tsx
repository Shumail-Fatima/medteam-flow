...
const AdminTaskManagement: React.FC = () => {
  ...
  const getFilteredTasks = () => {
    switch (userRole) {
      case 'nurse':
        ...
        return tasks.filter(task => task.assigneeId === String(user?.id));
      case 'doctor':
      case 'admin':
  ...
  const canEditTask = (task: Task) => {
    ...
    if (userRole === 'nurse') return task.assigneeId === String(user?.id);
    return false;
  };
  ...
  const handleTaskSave = (taskData: TaskFormData) => {
    console.log('handleTaskSave called', taskData);
    if (selectedTask) {
      // Edit existing task
      ...
      const newTask: Task = {
        ...
        createdBy: user?.id ? String(user.id) : 'unknown',
